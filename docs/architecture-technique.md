...
- **Ports de données en loopback** : Postgres (5433), Redis (6380) et MinIO (9002, 9003) sont publiés sur `127.0.0.1` uniquement (`docker-compose.yml`) ; le pare-feu n'est qu'une seconde barrière. Postgres exige un mot de passe (SCRAM), Redis répond `NOAUTH` sans mot de passe.
- **En-têtes de sécurité des frontends publics** : `vite preview` (Admin et PWA) renvoie `Content-Security-Policy`, `X-Frame-Options: DENY`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy` et `Strict-Transport-Security`, avec les mêmes valeurs que `nginx.static.conf` (production). Vérifié dans un navigateur : connexion et navigation Admin, service worker PWA actif, Admin non affichable dans un cadre. Le script d'analytique injecté par Cloudflare (`static.cloudflareinsights.com`) est bloqué par la CSP, sans effet sur l'application.
